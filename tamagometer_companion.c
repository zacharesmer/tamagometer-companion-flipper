...
DecoderStates decoder_states = {
    ...
    .data_1_space = 1500,
    .data_1_space_tolerance =
        500, // match the min long gap to max short gap: 1000
    .ending_mark = 1100,
...
static void back_button_callback(void *context) {
  // TODO make sure that all the other stuff gets freed in case it's still
  // running from CLI (ir worker, timer)
...
bool decode_signal_to_tamabits(InfraredWorkerSignal *received_signal,
  ...
  if (!(MATCH_TIMING(timings[0], decoder_states.header_mark,
                     decoder_states.header_mark_tolerance) &&
        MATCH_TIMING(timings[1], decoder_states.header_space,
  ...
  for (size_t data_bit = 0; data_bit < 160; data_bit++) {
    if (MATCH_TIMING(timings[timing], decoder_states.data_mark,
                     decoder_states.data_mark_tolerance)) {
      if (MATCH_TIMING(timings[timing + 1], decoder_states.data_0_space,
                       decoder_states.data_0_space_tolerance)) {
        // It's a 0, store it
        data_bits[data_bit] = '0';
      } else if (MATCH_TIMING(timings[timing + 1], decoder_states.data_1_space,
...
static void signal_received_callback(void *context,
...
static void listen(Cli *cli, void *context) {
  ...
  // will be called and cause a null pointer dereference. Hmph. Longer term I
  // will need to redesign this I think
...
int32_t tamagometer_companion(void *arg) {
  ...
  cli_add_command(cli, "tamagometer", CliCommandFlagParallelSafe,
                  tamagometer_start_cli, NULL);
  ...
  text_box_set_text(text_box, "Connect to\n"
