 ...
 * It will also add the command "tamagometer" to the CLI for use with https://zacharesmer.github.io/tamagometer/
 */
...
static void back_button_callback(void* context) {
    // TODO make sure that all the other stuff gets freed in case it's still running from CLI (ir worker, timer)
...
// take in a signal from the IR worker, and decode it into a 160 bit tamagotchi bit-string
// and store that in the provided buffer.
// Return true if success, false if it was not a decodable message.
//
...
bool decode_signal_to_tamabits(InfraredWorkerSignal* received_signal, unsigned char* data_bits) {
    ...
    // Check if there are at least 323 timings, otherwise the message won't fit and there's
    // no possible way it's valid
    if(timings_cnt < 323) {
        return false;
    }
    ...
    // I could check that the last mark is the end mark length (longer than a data bit mark)
    // but if we got 160 data bits that's good enough
    return true;
}
...
static void listen(Cli* cli, void* context) {
    ...
    // Could add a flag that's set when a command starts processing and then make it wait until that's done. If it's successful, do nothing because it will print the decoded signal. If it was unsuccessful, print timed out message.
    // There is still a possibility that the signal is being received while the timer times out. What happens then? At best it will get lost, at worst the callback will be called
    // and cause a null pointer dereference. Hmph.
    // Longer term I will need to redesign this I think
